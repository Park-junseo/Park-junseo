# 👋 안녕하세요. 서비스 개발을 사랑하는 엔지니어, 박준서입니다!

> 기술로 세상을 연결하고, 서비스의 가치를 높이는 개발자입니다.  
> 프론트엔드와 백엔드를 아우르며, 확장 가능하고 효율적인 아키텍처를 지향합니다.

---

## 🌱 About Me

저는 **비즈니스의 본질을 기술로 해결하는 개발자**입니다.  
창업 경험을 통해 고객의 문제를 직접 마주했고, 그 과정에서  
"장황한 기술보다, 꼭 필요한 기술"이 더 중요하다는 것을 배웠습니다.

데이터의 구조부터 사용자 경험까지,  
**서비스 전체를 이해하고 설계하는 일**을 좋아합니다.  
서비스 기획부터 배포, 운영까지 모든 과정을 함께 하는 일에 흥미가 있습니다.

배우는 것을 두려워하지 않습니다.
오히려 즐겁습니다. **죽기 전까지 공부**하는 것이 소원입니다.

---

## 🌐 포트폴리오

📘 **Notion Portfolio**  
- [노션 직접 페이지](https://palm-red-c5a.notion.site/145f8efc41568090aa00c46927826478)
- [노션 기반 블로그](https://park-junseo.vercel.app/)

---

## 🛠 Tech Stack

| 분야 | 기술 스택 |
|------|-------------|
| **Backend** | NestJS 11.x / TypeScript / Node.js 20.x |
| **Frontend** | React / Next.js / React Native / Three.js |
| **Database** | PostgreSQL 15 / Prisma 6.x |
| **Search Engine** | Elasticsearch |
| **Message Queue** | Kafka / KSQLDB |
| **Cache & Pub-Sub** | Redis |
| **CDC** | Debezium |
| **Infra & Cloud** | Docker / Docker Compose / Kubernetes / Helm / Nginx / Certbot |
| **DevOps & Tools** | Git / GitHub Actions / CI·CD / Nx (Monorepo) |
| **ETC** | WebGL / Three.js / RESTful API / GraphQL / Socket.IO / JWT |

---

## 💼 Experience & Projects

### ☕ 카페 마케팅 플랫폼
> **마이크로서비스 아키텍처 / CDC 데이터 파이프라인 / 실시간 멀티플레이어 동기화**

git 링크: [https://github.com/Park-junseo/mecipe-was-repo](https://github.com/Park-junseo/mecipe-was-repo)
- **Nx 모노레포** 기반 마이크로서비스 아키텍처 설계 및 구현
- **CDC(Change Data Capture) 파이프라인** 구축: PostgreSQL → Debezium → Kafka → KSQLDB → Elasticsearch
- **Elasticsearch**를 활용한 고성능 카페 검색 기능 구현
- **어드민 페이지**: 카페, 카테고리, 이벤트 등록 및 관리 시스템
- **Three.js 기반 웹 VR 환경**: 3D 가상 공간에서 카페 탐색 및 체험
- **Socket.IO + Redis**: 다중 서버 환경에서 실시간 아바타 멀티플레이어 동기화
  - Redis 분산 락을 통한 리더 선별 및 브로드캐스트 최적화
  - 세션 기반 재연결 지원으로 안정적인 연결 관리
- **API Gateway**: JWT 인증 미들웨어 및 데코레이터 기반 라우팅 정책 관리
- **Docker Compose & Kubernetes**: 경량 모델과 Full 모델 아키텍처 지원
- **GitHub Actions**: Self-Hosted Runner를 통한 자동 배포 파이프라인 구축
- **Nginx + Certbot**: HTTPS 자동 인증서 발급 및 리버스 프록시 구성

### 🌐 WebGL 및 VR 콘텐츠 플랫폼
git 링크: [https://github.com/Newinderstudio/WebClient-Mecipe](https://github.com/Newinderstudio/WebClient-Mecipe)
> **Unity 및 Three.js / 성능 최적화 / 가상공간 서비스 구현**

- Unity 기반 콘텐츠를 **Three.js / WebGL**로 재구현하여 웹 기반 서비스화  
- AI 협업 툴과 오픈소스를 활용해 빠른 기술 습득 및 개발 속도 향상

### 🧩 욕실 수리 중개 플랫폼
> **입찰·낙찰 시스템 / PG 결제 연동 / 포인트 및 할인 로직**

- 수리 견적의 **입찰·낙찰 로직**과 **PG 결제 모듈**을 RESTful API로 설계  
- 트랜잭션 안정성을 확보하기 위한 **보상 트랜잭션 설계 및 테스트 케이스 작성**  
- 데이터 정합성과 보안을 중심으로 DB 스키마를 설계하여 실제 서비스 수준으로 구현


### 🕹️ 유니티 기반 3D 대규모 다중 사용자 온라인 샌드박스 메타버스 플랫폼
git 링크:[https://github.com/HBNU-SWUNIV/come-capstone24-nextreality](https://github.com/HBNU-SWUNIV/come-capstone24-nextreality)
> **[대학 캡스톤 디자인] Unity / UDP 통신 멀티플레이 / TCP 기반 P2P 에셋 다운로드**

- 에셋 업로드 및 UGC 설치
- 사용자 아바타 및 오브젝트 동기화
- P2P 통신을 통한 에셋 다운로드 중앙 집중 트래픽 완화

---

## 📊 Strengths

- **서비스 전체를 이해하는 통합적 개발 사고**
- **데이터 모델링 & API 구조 설계 능력**
- **마이크로서비스 아키텍처 설계 및 구현 경험**
- **CDC 파이프라인 및 실시간 데이터 동기화 구축 경험**
- **클라우드 운영 및 배포 자동화 경험 (Docker, Kubernetes, GitHub Actions)**
- **문제 상황에 대한 구조적 접근과 빠른 학습 속도**
- **협업과 커뮤니케이션 중심의 개발 문화 지향**

---

## 🤝 Collaboration & Communication

- Git Flow 기반의 협업 경험  
  → `fork`, `branch`, `Pull Request` 중심의 버전관리와 문서화  
- UI/UX
- 배우는 것을 좋아하다보니, Photoshop, Illustration, After Effect, Premiere Pro, Blender, Figma 등 다 쓸 줄 알게 되어버렸어~
